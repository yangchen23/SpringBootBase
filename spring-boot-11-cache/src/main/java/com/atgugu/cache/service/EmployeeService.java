...
public class EmployeeService {
     ...
     *      5、可以获取和创建ConcurrentMapCache类型的缓存组件：他的作用是将缓存保存在ConcurrentMap中：
     *
     * 运行的流程：
     * @Cacheable:
     *      1、方法运行之前，先去查询Cache（缓存组件），安装CacheNames指定的名字获取；
     ...
     *      如果没有就运行方法将即如果放入缓存；以后再来调用就可以直接使用缓存中的数据
     *
     * 核心：
     *      1）、使用CacheManager【ConcurrentMapCacheManager】按照名字得到Cache【ConcurrentMapCache】组件
     *      2）、key使用keyGenerator生成的，默认是SimpleKeyGenerator生成key
     *
     * @param id
     * @return
     */
     ...
     *
     *  beforeInvocation = true：代表清除缓存操作是在方法运行之前执行，无论方法是否出爱心异常，缓存都清除
     */
